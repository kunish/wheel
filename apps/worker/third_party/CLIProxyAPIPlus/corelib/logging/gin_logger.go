...
func GinLogrusRecovery() gin.HandlerFunc {
	// Use a raw middleware instead of gin.CustomRecovery so that re-panicking
	// with http.ErrAbortHandler actually escapes ServeHTTP.
	// gin.CustomRecovery's internal recovery catches the re-panic before it
	// can propagate out.
	return func(c *gin.Context) {
		defer func() {
			if recovered := recover(); recovered != nil {
				if err, ok := recovered.(error); ok && errors.Is(err, http.ErrAbortHandler) {
					// Let net/http handle ErrAbortHandler so the connection is
					// aborted without noisy stack logs.
